...
class Character:
    ...
    def special_attack(self, other):
        ...
        self.mp -= 10
        print(
            f"[{self.name}]의 공격스킬! 스.파.르.타! \t {other.name}에게 [{damage}]의 데미지를 입혔습니다."
        )
    ...
    def heal(self, other):
        ...
        print(f"[{self.name}]님의 회복스킬! 중.꺾.그.마! [{self.name}]님의 체력이 {heal}회복 되었습니다.")

    ####################### 방어스킬 중꺾마! def_##########################
    def defend(self, other):

        damage = other.damage - self.def_
        self.hp = max(self.hp - damage, 0)

        print(f"{self.name}의 방어! 중.꺾.마 ! {other.name}가 다음턴에 공격시 데미지가 감소합니다.")
    ...
    def counter(self, other):
        ...
        def timeover():
            print("입력시간이 초과되었습니다.")
            self.hp = max(self.hp - damage, 0)
            print(f"{self.name}님이 카운터 실패로 {damage}만큼의 역관광를 입었습니다.")
            return True

        timeout = 3
        t = Timer(timeout, timeover)
        t.start()
        prompt = f"\n {timeout} 초 안에 {other.name}을 입력하세요..!\n"
        print(prompt)
        ...
        if ans == other.name:
            t.cancel()
            other.hp = max(other.hp - damage, 0)
            print(f"{other.name}이(가) {damage}만큼의 카운터데미지를 입었습니다.")
        elif ans != other.name:
            t.cancel()
            self.hp = max(self.hp - damage, 0)
            print(f"{self.name}님이 카운터 실패로 {damage}만큼의 역관광를 입었습니다.")
